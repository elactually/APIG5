...
while True:
  ...
  # Print the divider line for better readability
       ...
       print("==================")
       # Iterate through each step in the directions and print the maneuver details
       for each in json_data['route']["legs"][0]["maneuvers"]:
            # Print the step's narrative and distance in kilometers (converted from miles)
            print(each['narrative'] + "(" + str("{:.2f}".format(each["distance"]*1.61))+"km)")
            # Print a separator after each maneuver for clarity
            print("==================\n")
       else:
            # If the status code is not 0, it means the route could not be found or there was an error
            print("API Status: " + str(json_status)+ " = Route not found or invalid input. Please try again.")
            # Print a separator to indicate an error or invalid input
            print("================================\n")
